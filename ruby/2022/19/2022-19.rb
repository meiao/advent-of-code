...
module Resources
  LIST = %i[geode obsidian clay ore]
end
...
class Blueprint
  ...
  def calculate(statuses, time)
    ...
    cur_statuses.map { |status| status.resources[:geode] }.max
  end

  def clean_statuses(statuses)
    if statuses.size > 10_000
      p statuses.size
      return statuses
    end
    ...
    until cur_statuses.empty?
      ...
      next_statuses << status if cur_statuses.none? { |s| status.worse_than?(s) }
    end
...
class Status
  ...
  def buy_statuses(status, costs, max_costs)
    ...
    resources.each do |resource|
      ...
      next unless costs[resource].all? { |k, v| @resources[k] >= v }
